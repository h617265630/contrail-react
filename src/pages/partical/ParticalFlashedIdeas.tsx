...
function loadItems(): ParticalItem[] {
  try {
    ...
    return parsed.filter(
      (i) => typeof i?.id === 'string' && typeof i?.kind === 'string' && typeof i?.title === 'string'
    )
  } catch {
    return []
  }
...
export default function ParticalFlashedIdeas() {
  ...
  const [draft, setDraft] = useState({ kind: 'link' as ItemKind, title: '', url: '', content: '', note: '' })
  ...
  const documents = useMemo(() => items.filter((i) => i.kind === 'document' && i.url), [items])
  const snippets = useMemo(() => items.filter((i) => i.kind === 'snippet' && i.content), [items])
  const notes = useMemo(() => items.filter((i) => i.kind === 'note' && i.content), [items])
  const links = useMemo(() => items.filter((i) => i.kind === 'link' && i.url), [items])
    ...
    <div className="min-h-[60vh] space-y-6">
      ...
      <section className="rounded-md border border-stone-200 bg-white p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-stone-500 mt-1">支持：链接 / 文档链接 / 笔记 / 文字片段</p>
          </div>
          <Button type="button" className="rounded-md" disabled={!canSave} onClick={saveItem}>
            保存
          </Button>
        </div>
        ...
        <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-stone-900 mb-2">类型</label>
            <select
              value={draft.kind}
          ...
          <div>
            <label className="block text-sm font-semibold text-stone-900 mb-2">标题 *</label>
            <input
              type="text"
              value={draft.title}
          ...
          {(draft.kind === 'link' || draft.kind === 'document') ? (
            <div className="lg:col-span-2">
              <label className="block text-sm font-semibold text-stone-900 mb-2">链接 *</label>
              <input
                type="url"
                value={draft.url}
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-semibold text-stone-900 mb-2">内容 *</label>
              <textarea
                value={draft.content}
          ...
          <div className="lg:col-span-2">
            <label className="block text-sm font-semibold text-stone-900 mb-2">备注（可选）</label>
            <textarea
              value={draft.note}
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-stone-500">{documents.length} items</span>
        </div>
        {documents.length === 0 ? (
            ...
            {documents.map((d) => (
              ...
              >
                <div className="min-w-0">
                  <h3 className="text-stone-900 font-semibold line-clamp-1" title={d.title}>
                    {d.title}
                  </h3>
                  <a
                    href={d.url}
                  ...
                  {d.note && (
                    <p className="text-stone-500 text-sm mt-1 line-clamp-2" title={d.note}>
                      {d.note}
                    </p>
                  )}
                  <p className="text-xs text-stone-500 mt-1">{formatDate(d.createdAt)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 rounded-md border border-stone-200 bg-stone-50 text-xs font-semibold text-stone-900">
                    Document
                  </span>
                  <Button type="button" size="sm" className="rounded-md" asChild>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-stone-500">{snippets.length} items</span>
        </div>
        {snippets.length === 0 ? (
          <div className="rounded-md border border-stone-200 bg-stone-50 p-6 text-sm text-stone-500">
            暂无文字片段
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {snippets.map((t) => (
              <article key={t.id} className="bg-white rounded-md border border-stone-200 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-stone-900 font-semibold line-clamp-1" title={t.title}>
                      {t.title}
                    </h3>
                    <p className="text-xs text-stone-500 mt-1">{formatDate(t.createdAt)}</p>
                  </div>
                  <span className="px-2 py-1 rounded-md border border-stone-200 bg-stone-50 text-xs font-semibold text-stone-900">
                    Text
                  </span>
                ...
                {t.note && (
                  <p className="mt-3 text-stone-500 text-sm line-clamp-2" title={t.note}>
                    {t.note}
                  </p>
                )}
                <div className="mt-4 flex items-center justify-between gap-3">
                  <div className="text-xs text-stone-500">{t.kind === 'snippet' ? 'Snippet' : 'Text'}</div>
                  <Button
                    type="button"
                    variant="destructive"
                    size="sm"
      ...
      <section className="space-y-4">
        ...
        {notes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {notes.map((n) => (
              <article key={n.id} className="bg-white rounded-md border border-stone-200 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-stone-900 font-semibold line-clamp-1" title={n.title}>
                      {n.title}
                    </h3>
                ...
                {n.note && (
                  <p className="mt-3 text-stone-500 text-sm line-clamp-2" title={n.note}>
                    {n.note}
                  </p>
                )}
        ...
        {links.length === 0 ? (
          ...
          <div className="space-y-3">
            {links.map((l) => (
              ...
              >
                <div className="min-w-0">
                  <h3 className="text-stone-900 font-semibold line-clamp-1" title={l.title}>
                    {l.title}
                  </h3>
                  <a
                    href={l.url}
                  ...
                  {l.note && (
                    <p className="text-stone-500 text-sm mt-1 line-clamp-2" title={l.note}>
                      {l.note}
                    </p>
                  )}
                  <p className="text-xs text-stone-500 mt-1">{formatDate(l.createdAt)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 rounded-md border border-stone-200 bg-stone-50 text-xs font-semibold text-stone-900">
                    Link
