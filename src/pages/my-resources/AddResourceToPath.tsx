...
import { listMyLearningPaths, addResourceToMyLearningPath } from '@/api/learningPath'
...
export default function AddResourceToPath() {
  ...
  async function handleAdd(pathId: number) {
    ...
    try {
      ...
      setError(String(e?.response?.data?.detail || e?.message || 'Failed to add'))
    } finally {
      setAddingId(null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        {loading && (
          <div className="text-center py-12 text-stone-400">Loading paths...</div>
        )}

        {!loading && paths.length === 0 && (
          <div className="text-center py-12">
            <p className="text-stone-500 mb-4">You have no learning paths yet.</p>
            <Button className="rounded-none bg-amber-500 text-white hover:bg-amber-600" onClick={() => navigate('/createpath')}>
              Create a Path
            </Button>
          </div>
        )}
            ...
            {paths.map((path) => (
              ...
              >
                <div>
                  ...
                  {path.description && (
                    <p className="text-sm text-stone-400 mt-1 line-clamp-1">{path.description}</p>
                  )}
                </div>
                <Button
                  size="sm"
